                                ...
                                const unsigned int noChnls)
{
    ...
    int* chnlSums_host = new int[noChnls];
    int* chnlNoPxs_host = new int[noChnls];
    ...
    for (int c = 0; c < noChnls; c++)
        outChnlMeans_host[c] = (chnlNoPxs_host[c] > 0) ? ((float)chnlSums_host[c] / chnlNoPxs_host[c]) : 0;

    // clean up
    cudaFree(chnlSums_dev);
    cudaFree(chnlNoPxs_dev);
    delete[] chnlSums_host;
    delete[] chnlNoPxs_host;
}
