...
static const prgrm wli_prgrm	= {{{1.f,0.85f,0.85f,1.f,0.85f,0.85f,1.f,0.85f,0.85f,0.f,0.f,0.f,0.f,0.f,0.f},	IMG_CHNL::MONO}};
...
static const prgrm multi_prgrm  = {{{0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,1.f,0.f,0.f,0.f,0.f,0.f},				IMG_CHNL::BLUE},
								   {{0.f,0.7f,0.f,0.f,1.0f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f},			IMG_CHNL::GREEN},
...
void LightController::SetMode(MODE mode)
{
	...
	else
	{
		...
		switch (mMode)
		{
			case MODE::WLI:		mPrgrm = wli_prgrm;		break;
...
void LightController::IncrementPrgrm(float* oddBgrVals, float* evenBgrVals)
{
	...
	// if we are sync mode and we are not yet synchronized
	...
	else
	{
		...
		for (int f = 0; f < 2; f++)
		{
			...
			if (mPrgrmCount >= mBufferOffset)
			{
				...
				float vals[3];
				if (f) memcpy(vals, evenBgrVals, 3 * sizeof(float));
				else   memcpy(vals, oddBgrVals, 3 * sizeof(float));

				float prevIntensity = 0.f;
				switch (mPrgrm[frameIdx].second)
				{
				case IMG_CHNL::BLUE:    prevIntensity = vals[0];								break;
				case IMG_CHNL::GREEN:   prevIntensity = vals[1];								break;
				case IMG_CHNL::RED:     prevIntensity = vals[2];								break;
				case IMG_CHNL::MONO:    prevIntensity = (vals[0] + vals[1] + vals[2]) / 3.f;	break;
				}
				...
				// Get the illumination power value that 
				// produced this image intensity
				float prevPwr = 0.f;
			...
			// if we have exceeded the buffer offset and we have made a full pass through 
			// the program, then start doing autoexposure.
			float newPwr = 0.f;
			if (mPrgrmCount >= mPrgrm.size() * (int)ceilf((float)mBufferOffset / (float)mPrgrm.size()))
				mFrmBuffer.try_dequeue(newPwr);
			// otherwise, use the default starting power value
			else
				newPwr = PWR_START;
	...
	// instead, we will adjust the rotation mount angle
	if (mMode == MODE::LSCI)
	{
		pws[29] = PW_LSCI;
	...
	// recieve photodiode voltages from teensy if available
	input_entry ientry;
	if (mThreadedSerial->GetFromRxQueue(ientry))
	{
		// if we recieve an error from the teensy,
		// log an error
